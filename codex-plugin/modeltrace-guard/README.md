...
按工具调用次数，使用原 Codex 任务的冻结历史快照创建临时 fork 进行指纹检查；主任务只接收评分摘要，不接收探针数字数组。评分使用插件附带的 ModelTrace 指纹库。探针由 Codex 自身生成，使用其已配置的账户、模型与厂商，消耗对应推理额度，不需要另填 API Key。
...
需要 Node.js 18+，以及支持原生快照 fork、临时任务、任务删除和 hooks 的 Codex App Server。可用 `node scripts/guard.mjs doctor --fork true` 检查兼容性。若系统有多个运行时，将 `MODELTRACE_CODEX_PATH` 设置为 Codex 桌面应用配套可执行文件的绝对路径。

### 加载与信任

1. 安装后打开一个新 Codex 任务加载插件。
2. 在终端运行兼容的 `codex`，进入其交互输入框后输入 `/hooks`。这不是 PowerShell 命令。
3. 找到 ModelTrace Guard，审阅并信任所需 hooks，包括 `PreToolUse` 工具拦截。插件更新后，如果 hook 定义发生变化，需要重新审阅。
...
`doctor --fork true` 只检查原生分支和清理能力，不发起推理。`start` 返回 `probe --challenge <id>` 命令，由智能体执行，无需手动填写数字。外部终端手动开启时可执行返回的探针命令，并将结果中的异常告知要求传回目标任务；后续自动调度依赖目标任务已加载且受信任的 hooks。
...
没有每轮或任务累计探针数量上限。普通检查按工具间隔进行，空闲不采样，同一任务最多一个在途探针。修改设置不清空历史；当前复测组保持创建时的配置。

## 同一快照、独立 fork、测后清理

1. 由 Codex 原生 `thread/fork` 创建一个持久化临时基准；它不运行模型、不接受后续工作。
2. 固定基准的轮次边界；首次探针与 N 次复测分别对它调用 `thread/fork`，且每次设置 `ephemeral: true` 和相同 `lastTurnId`。
3. 子 fork 只输出一个数组，受信任 hook 拦截其工具调用。执行器读取结果、关闭临时 Codex 进程，再离线评分；原任务只得到候选、时间、快照摘要与通知要求。
4. 普通检查结束或整组复测结束后，独立清理进程核对所属任务与轮次，通过 `thread/delete` 删除基准。临时基准可能短暂出现在任务列表中，请勿把它用于工作。被使用或产生其他持久化后代的基准会保留并报告清理阻塞，避免连带删除用户内容。

每次复测都读取首次检测使用的快照，不继承其他探针的数字，也不包含告警后主任务增加的内容。原任务的已有消息保持不变。

快照包含 Codex 已保存的任务历史，不包含尚未落盘的内容；运行中的轮次可能带有 Codex 的中断标记。缓存命中还取决于客户端工具、配置、路由和缓存策略。样本详情中的 `fork.usage.cachedInputTokens` 展示 Codex 返回的缓存输入计数，`null` 表示未提供计数。

## 提醒与停止

首次候选不一致立即写入提醒，要求智能体先告诉用户“预期 X，指纹第一候选 Y”，再确认已告知。之后连续完成用户设定的 N 次复测，不等待普通间隔，也不递归再开启一组。所有 N 次均不一致才锁定任务；其中任一次相符则本组不触发停止，但异常记录保留。缺失、无效、超时或中断不计为不一致。
...
自动化测试覆盖协议、调度、同快照复测、清理边界、状态迁移、分页、告警和工具拦截；`verify-parity.mjs` 检查评分实现的一致性。模型归因准确率需使用独立验证集评估。
